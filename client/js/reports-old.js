class Reports extends Page {
	...
	static async loadState(state) {
		...
		if(location.pathname.includes('/reports-old/') && Reports.list.has(what))
			return Reports.list.get(what).edit();

		if(location.pathname.includes('/visualization-old/') && what) {
	...
	static async setup(container) {
		...
		Reports.container.querySelector('#add-report').on('click', () => {
			Report.add();
			history.pushState({id: 'add'}, '', `/reports-old/add`);
		});
...
class Report {
	...
	static import() {
		...
		form.on('submit', async (e) => {
			...
			history.pushState({what: response.insertId}, '', `/reports-old/${response.insertId}`);
		});
	}
	...
	get row() {
		...
		this.container.querySelector('.green').on('click', () => {
			...
			this.edit();
			history.pushState({what: this.query_id}, '', `/reports-old/${this.query_id}`);
		});
...
class ReportVisualization {
	...
	static setup() {
		...
		ReportVisualizations.preview.querySelector('#visualization-back').on('click', () => {
			...
			history.pushState(null, '', `/reports-old/${Report.selected ? Report.selected.id : ''}`);
		});
	...
	get row() {
		...
		row.querySelector('.green').on('click', () => {
			this.edit();
			history.pushState(null, '', `/visualization-old/${this.id}`);
		});
